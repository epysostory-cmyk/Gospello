...
const CSS = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-logo { width:32px; height:32px; border-radius:8px; object-fit:cover; flex-shrink:0; background:#e5e7eb; }
  ...
  .event-row {
    ...
    border-bottom:1px solid #f9f9f9; transition:background 0.1s;
    cursor:pointer;
  }
  ...
  .date-month { font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:0.05em; color:#6366f1; }
  ...
  .dot { display:inline-block; width:6px; height:6px; border-radius:50%; background:#6366f1; margin-right:3px; }
`
...
export async function GET(_req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!church) {
    return new Response('<html><body style="font-family:sans-serif;padding:20px;color:#9ca3af;">Church not found.</body></html>', {
      status: 404,
  ...
  const logoHtml = church.logo_url
    ? `<img src="${church.logo_url}" alt="${church.name}" class="header-logo" />`
    : `<div class="header-logo-fallback">${church.name[0]?.toUpperCase() ?? '?'}</div>`
...
<body>
  <div class="header">
    ${logoHtml}
    <div>
      <div class="header-name">${church.name}</div>
      <div class="header-sub">Upcoming Events</div>
    </div>
  </div>
  <div class="list">${eventsHtml}</div>
  <div class="footer">
    <a href="${siteUrl}/churches/${church.slug}" target="_blank" rel="noopener noreferrer" class="footer-link">
