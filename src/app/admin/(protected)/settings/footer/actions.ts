...
export async function getFooterSettings(): Promise<FooterSettings> {
  try {
    ...
    footer_social: (map['footer_social'] as FooterSettings['footer_social']) ?? { instagram: '', twitter: '', facebook: '', youtube: '', tiktok: '', whatsapp: '' },
    footer_copyright: (map['footer_copyright'] as string) ?? '© {year} Gospello. All rights reserved.',
    footer_contact_email: (map['footer_contact_email'] as string) ?? 'hello@gospello.com',
    footer_bottom_links: (map['footer_bottom_links'] as { label: string; url: string }[]) ?? [
...
export async function saveFooterSettings(formData: FormData) {
  ...
  const upserts = [
    ...
    { key: 'footer_social', value: social as unknown as string, updated_at: new Date().toISOString() },
    { key: 'footer_columns', value: columns as unknown as string, updated_at: new Date().toISOString() },
    { key: 'footer_bottom_links', value: bottomLinks as unknown as string, updated_at: new Date().toISOString() },
  ]
