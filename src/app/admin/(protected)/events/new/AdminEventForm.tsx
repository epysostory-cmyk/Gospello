...
export default function AdminEventForm({ adminId, profiles }: Props) {
  ...
  useEffect(() => {
    ...
    setScheduleMap(prev => {
      const next: Record<string, { label: string; sessions: EventSession[] }> = {}
            ...
            <div className="space-y-4">
              ...
              {!tooLong && dateRange.length > 0 && (
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {dateRange.length > 1 && (
                        <label className="flex items-center gap-1.5 cursor-pointer select-none">
                          <div
                            onClick={() => {
                              const on = !sameAdminSchedule
                              setSameAdminSchedule(on)
                              if (on && dateRange.length > 1) {
                                ...
                                setScheduleMap(prev => {
                                  const next = { ...prev }
                                  for (const d of dateRange) next[d] = { ...prev[d] ?? { label: '' }, sessions: srcSessions.map(s => ({ ...s })) }
                                  return next
                                })
                              }
                            }}
                            className={`relative w-8 h-4 rounded-full transition-colors cursor-pointer ${sameAdminSchedule ? 'bg-[#7C3AED]' : 'bg-gray-200'}`}
                          >
                            <span className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform ${sameAdminSchedule ? 'translate-x-4' : 'translate-x-0'}`} />
                          </div>
                  ...
                  {/* SAME SCHEDULE MODE */}
                  {sameAdminSchedule && dateRange.length > 0 && (() => {
                    const sharedEntry = scheduleMap[dateRange[0]] ?? { label: '', sessions: [{ title: null, start_time: null, end_time: null, speaker: null }] }
                    const sharedSessions = sharedEntry.sessions?.length ? sharedEntry.sessions : [{ title: null, start_time: null, end_time: null, speaker: null }]

                    const updateShared = (sIdx: number, field: keyof EventSession, value: string) => {
                      setScheduleMap(prev => {
                        const next = { ...prev }
                        for (const d of dateRange) {
                          const cur = next[d] ?? { label: '', sessions: [] }
                          next[d] = { ...cur, sessions: cur.sessions.map((s, i) => i === sIdx ? { ...s, [field]: value || null } : s) }
                        }
                        return next
                      })
                    }
                    const addShared = () => {
                      setScheduleMap(prev => {
                        ...
                        for (const d of dateRange) {
                          const cur = next[d] ?? { label: '', sessions: [] }
                          next[d] = { ...cur, sessions: [...cur.sessions, { title: null, start_time: null, end_time: null, speaker: null }] }
                        }
                        return next
                      })
                    }
                    const removeShared = (sIdx: number) => {
                      setScheduleMap(prev => {
                        const next = { ...prev }
                        for (const d of dateRange) {
                          const cur = next[d] ?? { label: '', sessions: [] }
                          const updated = cur.sessions.filter((_, i) => i !== sIdx)
                          next[d] = { ...cur, sessions: updated.length ? updated : [{ title: null, start_time: null, end_time: null, speaker: null }] }
                        }
                        return next
                      })
                    }
                            ...
                            <div key={sIdx} className="border border-gray-100 rounded-lg p-3 space-y-2 bg-gray-50/50">
                              <div className="flex items-center justify-between">
                                ...
                                {sharedSessions.length > 1 && (
                                  <button type="button" onClick={() => removeShared(sIdx)} className="text-xs text-red-400 hover:text-red-600">Remove</button>
                                )}
                              </div>
                              <input type="text" value={session.title ?? ''} onChange={e => updateShared(sIdx, 'title', e.target.value)}
                                placeholder="Session name (e.g. Morning Service)"
                                className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#7C3AED] bg-white" />
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  <label className="block text-[10px] font-medium text-gray-400 mb-1">Start Time (optional)</label>
                                  <input type="time" value={session.start_time ?? ''} onChange={e => updateShared(sIdx, 'start_time', e.target.value)}
                                ...
                                <div>
                                  ...
                                  <input type="time" value={session.end_time ?? ''} onChange={e => updateShared(sIdx, 'end_time', e.target.value)}
                                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#7C3AED] bg-white" />
                                  {session.end_time && <p className="text-xs text-gray-500 mt-0.5">{fmt12(session.end_time)}</p>}
                                </div>
                              </div>
                              <input type="text" value={session.speaker ?? ''} onChange={e => updateShared(sIdx, 'speaker', e.target.value)}
                                placeholder="Speaker / Minister (optional)"
                                className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#7C3AED] bg-white" />
                            </div>
                          ))}
                          <button type="button" onClick={addShared}
                            className="w-full py-2 rounded-lg border border-dashed border-[#7C3AED]/40 text-sm font-semibold text-[#7C3AED] hover:bg-violet-50 transition-colors">
                            + Add Session
                          </button>
                        </div>
                      </div>
                  ...
                  {/* INDIVIDUAL DAY MODE */}
                  {!sameAdminSchedule && dateRange.map((date, idx) => { // eslint-disable-line
                    ...
                    const updateAdminSession = (sIdx: number, field: keyof EventSession, value: string) => {
                      setScheduleMap(prev => {
                        const cur = prev[date] ?? { label: '', sessions: [] }
                        const updatedSessions = cur.sessions.map((s, i) =>
                          i === sIdx ? { ...s, [field]: value || null } : s
                        )
                        return { ...prev, [date]: { ...cur, sessions: updatedSessions } }
                      })
                    }

                    const addAdminSession = () => {
                      setScheduleMap(prev => {
                        const cur = prev[date] ?? { label: '', sessions: [] }
                    ...
                    const removeAdminSession = (sIdx: number) => {
                      setScheduleMap(prev => {
                        const cur = prev[date] ?? { label: '', sessions: [] }
                        const updated = cur.sessions.filter((_, i) => i !== sIdx)
                      ...
                      <div key={date} className={`rounded-xl border-2 overflow-hidden transition-colors ${hasContent ? 'border-[#7C3AED]/30' : 'border-gray-100'}`}>
                        <div className={`flex items-center gap-3 px-4 py-2.5 ${hasContent ? 'bg-violet-50' : 'bg-gray-50'}`}>
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[11px] text-gray-400">{sessions.filter(s => s.title || s.start_time).length} session{sessions.filter(s => s.title || s.start_time).length !== 1 ? 's' : ''}</span>
                            {dateRange.length > 1 && (
                              <button
                                type="button"
                                onClick={() => {
                                  setScheduleMap(prev => {
                                    const src = prev[date] ?? { label: '', sessions }
                                    const next = { ...prev }
                                    for (const d of dateRange) next[d] = { ...next[d] ?? { label: '' }, sessions: src.sessions.map(s => ({ ...s })) }
                                    return next
                                  })
                                }}
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          <input
                            ...
                            placeholder="Day theme (optional — e.g. Workers Retreat)"
                            className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#7C3AED] bg-white"
                          />
                          ...
                          {sessions.map((session, sIdx) => (
                            <div key={sIdx} className="border border-gray-100 rounded-lg p-3 space-y-2 bg-gray-50/50">
                              <div className="flex items-center justify-between">
                                <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Session {sIdx + 1}</span>
                                {sessions.length > 1 && (
                                  <button type="button" onClick={() => removeAdminSession(sIdx)} className="text-xs text-red-400 hover:text-red-600">Remove</button>
                                )}
                              </div>
                              <input
                                type="text"
                                value={session.title ?? ''}
                                onChange={e => updateAdminSession(sIdx, 'title', e.target.value)}
                                placeholder="Session name (e.g. Morning Service)"
                                className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#7C3AED] bg-white"
                              />
                              <div className="grid grid-cols-2 gap-2">
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={e => updateAdminSession(sIdx, 'start_time', e.target.value)}
                                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#7C3AED] bg-white"
                                  />
                                  {session.start_time && <p className="text-xs text-[#7C3AED] mt-0.5">{fmt12(session.start_time)}</p>}
                                </div>
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={e => updateAdminSession(sIdx, 'end_time', e.target.value)}
                                    className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:border-[#7C3AED] bg-white"
                                  />
                                  {session.end_time && <p className="text-xs text-gray-500 mt-0.5">{fmt12(session.end_time)}</p>}
                                </div>
                              ...
                              <input
                                ...
                                onChange={e => updateAdminSession(sIdx, 'speaker', e.target.value)}
                                placeholder="Speaker / Minister (optional)"
                                className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-[#7C3AED] bg-white"
                              />
                            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {([
              ...
              .filter(([,, onlineOk]) => !form.is_online || onlineOk)
              .map(([key, label]) => (
