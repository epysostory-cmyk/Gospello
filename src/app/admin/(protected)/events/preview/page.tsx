...
const STATUS_CONFIG: Record<string, { bg: string; dot: string; text: string; label: string; desc: string }> = {
  pending:  { bg: 'bg-amber-50 border-amber-200',  dot: 'bg-amber-400',  text: 'text-amber-700', label: 'Pending Review', desc: 'Awaiting admin approval before going public' },
  approved: { bg: 'bg-green-50 border-green-200',  dot: 'bg-green-500',  text: 'text-green-700', label: 'Approved',       desc: 'Live and visible to the public' },
  rejected: { bg: 'bg-red-50   border-red-200',    dot: 'bg-red-500',    text: 'text-red-700',   label: 'Rejected',       desc: 'Not visible to the public' },
  hidden:   { bg: 'bg-gray-100 border-gray-200',   dot: 'bg-gray-400',   text: 'text-gray-600',  label: 'Hidden',         desc: 'Temporarily removed from public view' },
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          target="_blank"
          className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-900 border border-gray-200 hover:border-gray-300 px-3 py-1.5 rounded-lg transition-colors"
        >
      ...
      <div className="flex flex-col xl:flex-row gap-6 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          {event.banner_url ? (
            ...
            <div className="w-full aspect-video rounded-2xl bg-gray-100 border border-gray-200 flex items-center justify-center">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500">
              Organised by <span className="text-gray-700 font-medium">{profile?.display_name ?? 'Unknown'}</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            {event.external_link && <Detail label="External Link" value="Linked" icon={<Globe className="w-4 h-4" />} href={event.external_link} />}
            {event.tags?.length > 0 && <Detail label="Tags" value={event.tags.join(', ')} icon={<Tag className="w-4 h-4" />} />}
          </div>
          ...
          {event.rejection_reason && (
            <div className="bg-red-50 border border-red-200 rounded-2xl p-5">
              ...
              <p className="text-sm text-red-700 leading-relaxed">{event.rejection_reason}</p>
            </div>
          )}
        </div>
        ...
        <div className="w-full xl:w-72 xl:sticky xl:top-6 space-y-4 shrink-0">
          ...
          <div className={`border rounded-2xl p-4 ${status.bg}`}>
            ...
            <p className="text-xs text-gray-500 ml-4">{status.desc}</p>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-4 space-y-2.5">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Info</p>
