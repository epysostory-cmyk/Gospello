...
import { Plus, Search, CheckCircle, Clock, AlertCircle, ExternalLink, Pencil, Mic2, Users } from 'lucide-react'
import Link from 'next/link'
import { formatDate } from '@/lib/utils'
import Image from 'next/image'
import ProfileCardActions from '../seededchurches/ProfileCardActions'
...
export default async function AdminOrganizersPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { data } = await adminClient
    ...
    .select('id, name, slug, city, state, logo_url, is_claimed, verified_badge, claim_requested_at, contact_person, created_at, is_hidden')
  ...
  const claimedCount   = rows.filter(r => r.is_claimed).length
  const pendingCount   = rows.filter(r => !r.is_claimed && r.claim_requested_at).length
  const unclaimedCount = rows.filter(r => !r.is_claimed && !r.claim_requested_at).length
  ...
  function buildUrl(overrides: Partial<SearchParams>) {
    ...
    return `/admin/organizers${qs ? '?' + qs : ''}`
  }

  function ClaimBadge({ row }: { row: typeof rows[number] }) {
    ...
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Admin-created organizer profiles not tied to user accounts</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {[
          { label: 'Total',     value: (data ?? []).length, icon: Users,        bg: 'bg-blue-50',   color: 'text-blue-600',   href: buildUrl({ status: 'all' }) },
          { label: 'Claimed',   value: claimedCount,        icon: CheckCircle,  bg: 'bg-green-50',  color: 'text-green-600',  href: buildUrl({ status: 'claimed' }) },
          { label: 'Unclaimed', value: unclaimedCount,      icon: AlertCircle,  bg: 'bg-amber-50',  color: 'text-amber-600',  href: buildUrl({ status: 'unclaimed' }) },
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
        <div className="p-4 border-b border-gray-100">
          <form method="GET" action="/admin/organizers" className="flex gap-2">
            ...
            <button type="submit" className="px-4 py-2 rounded-xl bg-[#7C3AED] text-white text-sm font-semibold hover:bg-[#6D28D9] transition-colors flex-shrink-0">Search</button>
            {q && (
              <Link href={buildUrl({ q: '', page: '1' })}
                className="px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-500 hover:bg-gray-50 flex-shrink-0">Clear</Link>
            )}
          </form>
        </div>
      ...
      <p className="text-xs text-gray-400 px-1">
        {total} organizer{total !== 1 ? 's' : ''}{q && ` matching "${q}"`}
      </p>

      {/* Cards */}
      {pageRows.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-16 text-center shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
          <Mic2 className="w-8 h-8 text-gray-200 mx-auto mb-2" />
          <p className="text-sm text-gray-400">No organizers found.</p>
          ...
          {pageRows.map(row => (
            <div key={row.id} className={`bg-white rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4 ${row.is_hidden ? 'opacity-60' : ''}`}>
              <div className="flex items-start gap-3 mb-3">
                <div className="w-11 h-11 rounded-xl flex-shrink-0 overflow-hidden bg-blue-50 flex items-center justify-center text-xl">
                  {row.logo_url
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {row.contact_person && (
                    <p className="text-xs text-gray-400 mt-0.5 truncate">Contact: {row.contact_person}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 pt-3 border-t border-gray-50 flex-wrap">
                <Link href={`/organizers/${row.id}`} target="_blank"
                  className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-900 transition-colors mr-auto">
                  <ExternalLink className="w-3 h-3" /> View
                </Link>
                <ProfileCardActions id={row.id} type="organizer" name={row.name} isHidden={row.is_hidden} />
                <Link href={`/admin/seededchurches/organizer/${row.id}/edit`}
